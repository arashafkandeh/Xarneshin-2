...
from modules.nodes import nodes_bp
from assets.getinfo import getinfo_bp
from modules.overview import overview_bp
...
try:
    sentry_sdk.init(
        dsn="https://53bc07d7142846d8a9f36356413c65c0@app.glitchtip.com/11984",
        integrations=[FlaskIntegration()],
        release="xenon-panel@1.1.0",
        send_default_pii=True,
        traces_sample_rate=0.2, 
    )
except Exception as e:
    logger.warning(f"Sentry SDK could not be initialized. Error reporting is disabled. Error: {e}")
...
def view_balancers(node_id):
    ...
    if not config_data or "config" not in config_data:
        ...
        return redirect(url_for("nodes.show_nodes", node_id=node_id))
...
if __name__ == "__main__":
    # Register the new blueprint for node management
    app.register_blueprint(nodes_bp)
    
    # Register the existing blueprint for getinfo
    app.register_blueprint(getinfo_bp)

    # Register the overview blueprint
